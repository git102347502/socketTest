...
namespace SocketDemo
{
    public class SocketManager
    {
        ...
        public List<AsyncUserToken> m_clients= new List<AsyncUserToken>(); //客户端列表  
        ...
        private void ShowClient(int count, AsyncUserToken userToken)
        {
            if (count<0)
            {
                Msg.BeginInvoke(DateTime.Now.ToString() +"\r\n"+ userToken?.Remote + "客户端已登出！" + "\r\n", null,null);
                DeteList.BeginInvoke(userToken?.Remote.ToString(), null, null);
            }
            else
            {
                if (userToken != null)
                {
                    if (m_clients.Find(c=>c.Remote== userToken.Remote) ==null)
                    {
                        m_clients.Add(userToken);
                    }
               
                    Msg.BeginInvoke(DateTime.Now.ToString() + "\r\n" + userToken?.Remote + "客户端已连接！" + "\r\n", null, null);
                    MsgList.BeginInvoke(userToken?.Remote.ToString(),null,null);
                }
            }
        }

        private void GetMsg(AsyncUserToken userToken, byte[] msg)
        {
            if (userToken != null)
            {
                Msg.BeginInvoke(DateTime.Now.ToString() + " Clinet:" + userToken?.Remote + "\r\n"+System.Text.Encoding.Default.GetString(msg) + "\r\n", null, null);
        ...
        /// <summary>
        /// 注册操作以接受来自客户端的连接请求,在服务器监听套接字上发出接受操作时使用的上下文对象
        /// </summary>
        ...
        public void SendMessage(AsyncUserToken token, byte[] message)
        {
            ...
            {
                Msg.Invoke("SendMessage - Error:" + e.Message);
            }
        }
    }
}
